...
4. Аватар только MCP → `assets/avatar_clips/<id>/` → `heygen.source=prepared`. Live HeyGen API из чата не звать. Лук, движок и разрешение — из `config/config.yaml` (`heygen.avatar_id`, `heygen.engine`, `heygen.output`), **не отсюда**: id, переписанный в документации, устаревает молча, и ролик собирается на прежнем аватаре. **Аватар заказчика — по умолчанию в каждом ролике** (решение 26.09); `meta.avatar_mode: "none"` — только когда заказчик сам скажет «без аватара».
...
- Аватар — HeyGen MCP, по умолчанию в каждом ролике (без него — только по слову заказчика).
