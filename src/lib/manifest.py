...
* SFX — до 20 файлов (курируемая), музыка — до 15 (курируемая), мемы — 100.
...
class FootageIndex:
    ...
    def add(self, record: AssetRecord) -> AssetRecord:
        """Записать материал в общую базу — если ему там место.

        Синтетика мок-режима сюда не попадает. База лежит в репозитории и
        просматривается раньше внешних стоков (§7.2.1), а мок-прогон CI гоняется
        на каждом коммите: к этому моменту в базе накопилось 195 мок-записей из
        213, то есть 92 % «материала», которого не существует ни на одном диске.
        Живому ролику такая запись даёт только промах: индекс говорит «материал
        есть», файла нет, слот уходит в генерацию. Она же портит и обучение —
        предпочтения §11.3 считаются по тому же индексу.
        """
        if record.mock:
            return record
        self.items.append(record)
