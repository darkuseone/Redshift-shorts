...
167 шаблонов каталога — это не 167 реализаций, а набор рендереров с параметрами.
...
def _mark_accent(text: str, accent: str) -> str:
    ...
    if not accent or accent.upper() not in text.upper():
        return _esc(text)
    if accent.strip().upper() == text.strip().upper():
        return _esc(text)
...
def tr_whip_pan_shader(ctx: "TemplateCtx") -> Piece:
    ...
    Здесь вуали ``#0a0a0b``/``#48bfe3``, стальной слой и полосы смаза, ``x``.
    Без canvas. Цвета SCENE A/B каталога — жест шейдера, не палитра канала.
...
from .domain_warp_dissolve import dwd_transition_css, tr_domain_warp_dissolve
from .flash_through_white import ftw_transition_css, tr_flash_through_white
from .ridged_burn import rb_transition_css, tr_ridged_burn
from .ripple_waves import rw_transition_css, tr_ripple_waves
...
TRANSITIONS: dict[str, Callable[[TemplateCtx], Piece]] = {
    ...
    "domain_warp_dissolve": tr_domain_warp_dissolve,
    "flash_through_white": tr_flash_through_white,
    "ridged_burn": tr_ridged_burn,
    "ripple_waves": tr_ripple_waves,
...
def transition_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        ".tr-cinematic-zoom .cz-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-cinematic-zoom .cz-to{background:#D7263D;mix-blend-mode:overlay}"
        ".tr-cinematic-zoom .cz-r{inset:-18%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(215,38,61,0.72) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-cinematic-zoom .cz-b{inset:-14%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(242,86,107,0.65) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ...
        "position:absolute;inset:0;display:block;opacity:0;"
        "transform-origin:50% 50%}"
        ".tr-glitch-shader .gs-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-glitch-shader .gs-to{background:#D7263D;mix-blend-mode:overlay}"
        ...
        ".tr-glitch-shader .gs-r{background:#D7263D;mix-blend-mode:screen}"
        ".tr-glitch-shader .gs-b{background:#F2566B;mix-blend-mode:screen}"
        ".tr-glitch-shader .gs-scan{position:absolute;left:-12%;width:124%;"
        "display:block;opacity:0;background:rgba(215,38,61,0.5);"
        "mix-blend-mode:overlay}"
        ".tr-glitch-shader .gs-block{position:absolute;display:block;opacity:0;"
        "background:rgba(242,86,107,0.35);mix-blend-mode:screen}"
        ...
        ".tr-gravitational-lens .gw-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-gravitational-lens .gw-to{background:#D7263D;mix-blend-mode:overlay}"
        ...
        "background:radial-gradient(circle,rgba(215,38,61,0.78) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-gravitational-lens .gw-b{inset:-12%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(142,22,39,0.62) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-gravitational-lens .gw-ghost{"
        ...
        "position:absolute;inset:0;display:block;opacity:0;"
        "transform-origin:50% 50%}"
        ".tr-light-leak .ll-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-light-leak .ll-to{background:#D7263D;mix-blend-mode:overlay}"
        ".tr-light-leak .ll-sage{background:#8E1627;mix-blend-mode:overlay}"
        ".tr-light-leak .ll-blob{inset:-48% -38% 18% 12%;border-radius:50%;"
        "background:radial-gradient(circle at 78% 22%,"
        "rgba(255,255,255,0.95) 0%,rgba(215,38,61,0.78) 34%,"
        "rgba(242,86,107,0.28) 58%,transparent 74%);"
        "mix-blend-mode:screen}"
        ".tr-light-leak .ll-hot{inset:-18% -12% 62% 48%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(242,86,107,0.9) 0%,transparent 62%);"
        "mix-blend-mode:screen}"
        ...
        "rgba(242,86,107,0) 38%,rgba(242,86,107,0.88) 50%,"
        "rgba(215,38,61,0.5) 58%,transparent 78%);"
        "mix-blend-mode:screen;transform-origin:50% 50%}"
        ".tr-light-leak .ll-o0{inset:-36% -18% 48% 28%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(215,38,61,0.4) 0%,transparent 70%);"
        "mix-blend-mode:screen}"
        ".tr-light-leak .ll-o1{inset:-22% -8% 38% 8%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(242,86,107,0.32) 0%,transparent 70%);"
        "mix-blend-mode:screen}"
        ...
        "position:absolute;inset:0;display:block;opacity:0;"
        "transform-origin:50% 50%}"
        ".tr-sdf-iris .si-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-sdf-iris .si-steel{background:#7A7D82;mix-blend-mode:overlay}"
        ...
        "transform-origin:50% 50%}"
        ".tr-sdf-iris .si-iris{background:#D7263D;mix-blend-mode:overlay}"
        ".tr-sdf-iris .si-ring{"
        "background:radial-gradient(circle,transparent 46%,"
        "rgba(242,86,107,0.92) 50%,transparent 54%);"
        "mix-blend-mode:screen}"
        ...
        "position:absolute;inset:0;display:block;opacity:0;"
        "transform-origin:50% 50%}"
        ".tr-thermal-distortion .td-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-thermal-distortion .td-to{background:#D7263D;mix-blend-mode:overlay}"
        ".tr-thermal-distortion .td-mist{background:#7A7D82;mix-blend-mode:overlay}"
        ...
        "rgba(242,86,107,0.92) 0%,rgba(215,38,61,0.55) 42%,transparent 70%);"
        "mix-blend-mode:screen}"
        ...
        "rgba(242,86,107,0.7) 50%,transparent 100%);"
        "mix-blend-mode:screen;transform-origin:50% 50%}"
        ...
        "position:absolute;inset:0;display:block;opacity:0;"
        "transform-origin:50% 50%}"
        ".tr-whip-pan .wp-from{background:#111214;mix-blend-mode:overlay}"
        ".tr-whip-pan .wp-to{background:#D7263D;mix-blend-mode:overlay}"
        ".tr-whip-pan .wp-steel{background:#7A7D82;mix-blend-mode:overlay}"
        ...
        "rgba(242,86,107,0) 12%,rgba(215,38,61,0.88) 48%,"
        "rgba(17,18,20,0.45) 78%,transparent 100%);"
        "mix-blend-mode:screen;transform-origin:50% 50%}"
        ...
        "letter-spacing:-0.02em;color:#111214;line-height:1}"
        ...
        "background:linear-gradient(120deg,#F7F5F3 0%,#F2566B 50%,#D7263D 100%);"
        ...
        "transform-origin:50% 50%}"
        ".tr-transitions-3d .t3-a{background:#111214}"
        ".tr-transitions-3d .t3-b{background:#D7263D;opacity:0}"
        ...
        "margin-top:12px}"
        ".tr-transitions-3d .t3-a .t3-label{color:#7A7D82}"
        ...
        "background:#7A7D82;transform-origin:50% 50%}"
        ...
        "flex-direction:column;align-items:center;justify-content:center;"
        "transform-origin:50% 50%}"
        ".tr-transitions-blur .tb-a{background:#111214}"
        ".tr-transitions-blur .tb-b{background:#D7263D;opacity:0}"
        ...
        "font-size:40px;font-weight:600;letter-spacing:6px;"
        "margin-top:12px}"
        ".tr-transitions-blur .tb-a .tb-label{color:#7A7D82}"
        ...
        "flex-direction:column;align-items:center;justify-content:center;"
        "transform-origin:50% 50%}"
        ".tr-transitions-cover .tc-a{background:#111214}"
        ".tr-transitions-cover .tc-b{background:#D7263D;opacity:0}"
        ...
        "opacity:0}"
        ".tr-transitions-cover .tc-wb{background:#8E1627}"
        ".tr-transitions-cover .tc-wa{background:#D7263D}"
        ...
        f"margin:-{height // 2}px 0 0 -{width // 2}px;background:#111214}}"
        ...
        "font-size:40px;font-weight:700;letter-spacing:6px;"
        "margin-top:12px}"
        ".tr-transitions-destruction .tds-a .tds-label{color:#7A7D82}"
        ...
        "flex-direction:column;align-items:center;justify-content:center;"
        "transform-origin:50% 50%}"
        ".tr-transitions-light .tlt-a{background:#111214}"
        ".tr-transitions-light .tlt-b{background:#D7263D;opacity:0}"
        ...
        "background:linear-gradient(135deg,rgba(215,38,61,0.6),transparent 60%);"
        "transform-origin:50% 50%}"
        ...
        "flex-direction:column;align-items:center;justify-content:center;"
        "transform-origin:50% 50%}"
        ".tr-transitions-other .tto-a{background:#111214}"
        ".tr-transitions-other .tto-b{background:#D7263D;opacity:0}"
...
def dataviz_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        # Графика идёт поверх футажа: подписи белые, цифры и данные — бирюза
        # (brand_roles.turquoise). Тёмные буквы на тёмном кадре не читались.
        "color:var(--color-bg-pure);text-shadow:0 4px 24px rgba(0,0,0,0.55)}"
        ...
        "border-radius:50%;background:var(--color-cyan-deep);overflow:hidden}"
        ".dv-ring i{position:absolute;left:50%;top:0;width:50%;height:100%;"
        "background:var(--color-cyan);transform-origin:left center;display:block}"
        ".dv-pct{font-family:var(--font-display);font-size:96px;color:var(--color-cyan)}"
...
def fs_shared_axis_z(ctx: "TemplateCtx") -> Piece:
    ...
    Inter 900 и ``#18181b`` как в каталоге; ``tone=accent`` → ``#D7263D``,
...
from .code_morph import cm_fullscreen_css, fs_code_morph  # noqa: E402
...
def _render_fullscreen_body(ctx: "TemplateCtx") -> Piece:
    ...
    if params.get("code_morph"):
        return fs_code_morph(ctx)
...
def ov_lt_accent_underline(ctx: "TemplateCtx") -> Piece:
    ...
    вне списка. Мятный ``#46e5b7`` — чужой бренд, черта канала ``#D7263D``.
...
def ov_lt_clean_bar(ctx: "TemplateCtx") -> Piece:
    ...
    ``#D7263D``. Montserrat как в каталоге. Твины на маске, tab и строках,
    не на ``.clip``.
    """
    params = ctx.params
    name = str(params.get("name") or params.get("content") or params.get("text")
...
def ov_lt_dark_card(ctx: "TemplateCtx") -> Piece:
    ...
    Каталог твинит ``tl.to`` после ``gsap.set`` и прячет обёртку через
    ``visibility``. Движок требует ``fromTo`` на вложенных узлах; ``visibility``
    вне списка. Золото ``#f5b942`` — чужой бренд, черта канала ``#D7263D``.
...
def overlay_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        ".fullscreen-text.invert .fs-slam-card{background:rgba(20,21,23,0.52);"
        "backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);"
        "border-color:rgba(215,38,61,0.45);"
        "box-shadow:0 22px 56px rgba(0,0,0,0.35),inset 0 1px 0 rgba(255,255,255,0.14)}"
        ".fullscreen-text .fs-slam-card .fs-num{color:var(--color-bg-pure);"
        "text-shadow:0 3px 18px rgba(0,0,0,0.55)}"
        ".fullscreen-text .fs-slam-card .fs-cap{color:#f7f8fa;"
        "text-shadow:0 2px 12px rgba(0,0,0,0.55)}"
        ".fullscreen-text .fs-slam-lead{display:block;font-weight:600;letter-spacing:0.04em;"
        "text-transform:uppercase;color:var(--color-text-soft);line-height:1.1;opacity:0.92;"
        "will-change:transform}"
        ".fullscreen-text.invert .fs-slam-lead{color:#A8ADB6;"
        "text-shadow:0 2px 10px rgba(0,0,0,0.45)}"
        ".fullscreen-text .fs-slam-punch{color:var(--color-accent);"
        "font-weight:900;letter-spacing:-0.03em}"
        ".fullscreen-text.invert .fs-slam-punch{color:#D7263D;"
        "text-shadow:0 4px 22px rgba(215,38,61,0.35),0 3px 18px rgba(0,0,0,0.55)}"
        ...
        ".fullscreen-text.invert .fs-num{color:var(--color-bg-pure);"
        "text-shadow:0 3px 18px rgba(0,0,0,0.55)}"
        ".fullscreen-text .fs-cap{display:block;font-family:var(--font-subtitle);"
        "font-size:48px;font-weight:800;text-transform:none;color:var(--color-ink);"
        "will-change:transform}"
        ".fullscreen-text.invert .fs-cap{color:#f7f8fa;"
        "text-shadow:0 2px 12px rgba(0,0,0,0.55)}"
        ...
        "opacity:0;will-change:transform;"
        "color:var(--color-ink);"
        "-webkit-text-stroke:2px rgba(247,248,250,0.85);"
        "paint-order:stroke fill;"
        "text-shadow:0 4px 22px rgba(0,0,0,0.55)}"
        ".fullscreen-text.invert .lbc-ch{color:var(--color-bg-pure);"
        ...
        "-webkit-text-stroke:0;transform-origin:50% 65%;"
        "text-shadow:0 0 24px rgba(215,38,61,0.45)}"
        ...
        "-webkit-text-stroke:1.5px rgba(247,248,250,0.9);"
        "paint-order:stroke fill;"
        ...
        "justify-content:center;background:transparent;color:#ffffff;"
        "font-family:Inter,system-ui,sans-serif;font-weight:900;"
        "letter-spacing:-0.04em;line-height:1}"
        ".fullscreen-text.fs-shared-axis-z.saz-paper{background:#F7F5F3;color:#111214}"
        ".fullscreen-text.fs-shared-axis-z.saz-accent{color:#D7263D}"
        ".fullscreen-text.fs-shared-axis-z.saz-accent{"
        "text-shadow:0 2px 0 rgba(0,0,0,0.85),0 8px 28px rgba(0,0,0,0.55);"
        "-webkit-text-stroke:1.5px rgba(0,0,0,0.65);paint-order:stroke fill}"
        ...
        ".fullscreen-text.fs-code-3d.invert{background:transparent;color:#e1e4e8}"
        ...
        ".fullscreen-text.fs-code-diff.invert{background:transparent;color:#e6edf3}"
        ...
        "padding:0;overflow:hidden;isolation:isolate;display:flex;"
        "align-items:stretch;justify-content:center;background:transparent;"
        "color:#ffffff;font-family:Inter,system-ui,sans-serif;font-weight:700;"
        "text-transform:none;letter-spacing:0}"
        ".fullscreen-text.fs-beat-freeze-cut.invert{background:transparent;color:#ffffff}"
        ...
        "radial-gradient(ellipse 80% 60% at 50% 40%,rgba(215,38,61,0.10) 0%,"
        "transparent 55%),radial-gradient(ellipse 50% 40% at 80% 80%,"
        "rgba(255,255,255,0.05) 0%,transparent 50%),transparent}"
        ...
        "box-shadow:0 40px 120px rgba(0,0,0,0.45),0 0 0 1px rgba(215,38,61,0.12),"
        ...
        "background:radial-gradient(circle,rgba(215,38,61,0.32) 0%,transparent 68%);"
        "pointer-events:none}"
        ...
        ".fullscreen-text .bfc-wave-path{fill:none;stroke:#D7263D;stroke-width:4;"
        "stroke-linecap:round;stroke-linejoin:round}"
        ".fullscreen-text .bfc-wave-fill{fill:#D7263D;opacity:0.12}"
        ...
        "background:linear-gradient(180deg,#D7263D 0%,rgba(215,38,61,0.25) 100%);"
        ...
        "background:#D7263D;padding:8px 16px;border-radius:999px}"
        ...
        "letter-spacing:0.22em;text-transform:uppercase;color:#D7263D}"
        ...
        "background:linear-gradient(135deg,#111214,#0a0a0c);"
        ...
        ".fullscreen-text .bfc-b-value.accent{color:#D7263D}"
        ...
        "text-transform:uppercase;color:#ffffff;"
        "text-shadow:0 0 40px rgba(215,38,61,0.45),0 8px 40px rgba(0,0,0,0.55);"
        "will-change:transform,opacity}"
        ".fullscreen-text .bfc-outline{position:absolute;inset:48px;border:3px solid "
        "#D7263D;border-radius:8px;box-shadow:0 0 0 1px rgba(215,38,61,0.25),"
        "inset 0 0 0 1px rgba(215,38,61,0.15),0 0 48px rgba(215,38,61,0.2);"
        ...
        "linear-gradient(90deg,transparent 0%,rgba(215,38,61,0.10) 48%,"
        "transparent 52%),radial-gradient(ellipse 40% 55% at 50% 48%,"
        "transparent 40%,rgba(10,10,11,0.28) 100%);opacity:0;"
        ...
        "color:#ffffff;background:#D7263D;padding:10px 18px;border-radius:6px;"
        ...
        "transparent 0%,rgba(215,38,61,0.18) 35%,rgba(17,18,20,0.35) 50%,"
        "rgba(215,38,61,0.18) 65%,transparent 100%);filter:blur(18px);"
        ...
        "background:rgba(247,245,243,0.82);color:var(--color-ink);"
        "backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);"
        "border:2px solid rgba(17,18,20,0.14);"
        "box-shadow:0 22px 56px rgba(0,0,0,0.22),inset 0 1px 0 rgba(255,255,255,0.35);"
        "max-width:100%;will-change:transform}"
        ".fullscreen-text.invert .fs-fact{background:rgba(20,21,23,0.52);color:#f7f8fa;"
        "backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);"
        "border-color:rgba(215,38,61,0.45);"
        "box-shadow:0 22px 56px rgba(0,0,0,0.35),inset 0 1px 0 rgba(255,255,255,0.14)}"
        ...
        ".article-scroll.theme-dark .hl{background:rgba(215,38,61,.28);"
        "box-shadow:0 0 0 6px rgba(215,38,61,.28)}"
        ...
        ".lt-au-rule{display:block;height:6px;border-radius:3px;background:#D7263D;"
        "transform-origin:0% 50%;will-change:transform}"
        ".lt-au-rule.no-red{background:rgba(201,204,207,0.72)}"
        ...
        ".lt-cb-tab{display:block;width:12px;flex-shrink:0;background:#D7263D;"
        "transform-origin:50% 0%;will-change:transform}"
        ".lt-cb-tab.no-red{background:rgba(201,204,207,0.72)}"
        ...
        "font-weight:700;color:#111214;line-height:1.06;letter-spacing:-0.015em;"
        "white-space:nowrap;will-change:transform,opacity}"
        ".lt-cb-role{display:block;font-family:'Montserrat',var(--font-subtitle),sans-serif;"
        "font-weight:400;color:#7A7D82;line-height:1.2;letter-spacing:0.01em;"
        "white-space:nowrap;will-change:transform,opacity}"
        ...
        "background:rgba(20,21,23,0.52);backdrop-filter:blur(18px);"
        "-webkit-backdrop-filter:blur(18px);border-radius:14px;"
        "padding:24px 38px 26px 32px;border:1px solid rgba(215,38,61,0.4);"
        ...
        "white-space:nowrap;will-change:transform,opacity}"
        ".lt-dc-rule{display:block;height:4px;border-radius:2px;background:#D7263D;"
        "transform-origin:0% 50%;will-change:transform}"
        ".lt-dc-rule.no-red{background:rgba(201,204,207,0.72)}"
        ".lt-dc-card.no-red{border-color:rgba(201,204,207,0.28)}"
...
def hero_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        "font-size:168px;line-height:1.06;text-transform:uppercase;"
        ...
        "border-radius:30px 30px 30px 8px;background:var(--color-bg-light)}"
        ...
        "padding:24px 30px;border-radius:30px 30px 30px 8px;background:var(--color-bg-light);"
        ...
        "background:var(--color-bg-light);display:flex;flex-direction:column;justify-content:center;"
        ...
        ".hero-slam .sl-line.accent{color:var(--color-accent)}"
        ...
        "line-height:1.06;letter-spacing:-0.02em;color:var(--color-ink);"
