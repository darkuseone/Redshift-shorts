...
def hero_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        # --- плита типа и всплывающая плашка курсора ---
...
# --- приёмы из ветки cursor/hyperframes-sci-templates-647c -------------------
# Ветка курсора отпочкована от первого коммита, и слить её файлом нельзя: в
# ней нет полутора сотен коммитов основной работы. Перенесены ровно те
# определения, которых здесь не было. `hero_burst` не перенесён намеренно:
# лучи за головой у нас заменены знаками о предмете речи (hero-icons).
...
def _mark_accent(text: str, accent: str) -> str:
    ...
    Акцент, совпавший со всей фразой, красит не слово, а строку — и §3.3.2
    перестаёт что-либо значить: выделять внутри фразы становится нечем. Фраза
    из одного слова и так выделена размером.
    """
...
def fs_scan_band(ctx: "TemplateCtx") -> Piece:
    ...
    x0 = 0
    x1 = frame_w
    # Кегль подбирается под длину фразы, а не берётся долей ширины кадра.
    # 13 % ширины — это clamp из каталога, и он не знает, сколько в строке
    # букв: «180 ГРАДУСОВ» при `white-space: nowrap` уезжало под обрез с обеих
    # сторон, чернила шли от 27-го до 1057-го пикселя при ширине 1080.
    available = float(ctx.params.get("available_px") or (frame_w - 2 * 90))
    size = fit_size(content.upper(), available, max(28, min(150, round(0.13 * frame_w))))
...
def overlay_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        # Гарнитура канала, а не Inter: Inter в проект не поставлен, шрифт
        # падал в system-ui, и строка выходила шире расчёта на 18 % — «180
        # ГРАДУСОВ» уезжало под обрез с обеих сторон (чернила 27..1057 при
        # ширине 1080). Проверка §14 пропускает только те гарнитуры, которые
        # лежат в assets/fonts с кириллицей и лицензией.
...
# --- реестры пополняются приёмами курсора ------------------------------------
# Словари объявлены выше самих функций, поэтому дописываются здесь: иначе имя
# рендерера в каталоге есть, а вызвать его нечем.
HERO["hero-type-slab"] = hero_type_slab
HERO["hero-plate-pop"] = hero_plate_pop
TRANSITIONS["zoom_through"] = tr_zoom_through
DATAVIZ["stat-countup-card"] = dv_stat_card
