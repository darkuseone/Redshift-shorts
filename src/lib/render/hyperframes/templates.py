...
def transition_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        ".tr-cinematic-zoom .cz-to{background:#E5322D;mix-blend-mode:overlay}"
        ".tr-cinematic-zoom .cz-r{inset:-18%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(229,50,45,0.72) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-cinematic-zoom .cz-b{inset:-14%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(255,106,95,0.65) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ...
        ".tr-glitch-shader .gs-to{background:#E5322D;mix-blend-mode:overlay}"
        ...
        ".tr-glitch-shader .gs-r{background:#E5322D;mix-blend-mode:screen}"
        ".tr-glitch-shader .gs-b{background:#FF6A5F;mix-blend-mode:screen}"
        ".tr-glitch-shader .gs-scan{position:absolute;left:-12%;width:124%;"
        "display:block;opacity:0;background:rgba(229,50,45,0.5);"
        "mix-blend-mode:overlay}"
        ".tr-glitch-shader .gs-block{position:absolute;display:block;opacity:0;"
        "background:rgba(255,106,95,0.35);mix-blend-mode:screen}"
        ...
        ".tr-gravitational-lens .gw-to{background:#E5322D;mix-blend-mode:overlay}"
        ...
        "background:radial-gradient(circle,rgba(229,50,45,0.78) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-gravitational-lens .gw-b{inset:-12%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(156,27,23,0.62) 0%,transparent 58%);"
        "mix-blend-mode:screen}"
        ".tr-gravitational-lens .gw-ghost{"
        "background:radial-gradient(circle,rgba(229,50,45,0.2) 0%,transparent 70%);"
        "mix-blend-mode:screen}"
        ...
        ".tr-light-leak .ll-to{background:#E5322D;mix-blend-mode:overlay}"
        ".tr-light-leak .ll-sage{background:#9C1B17;mix-blend-mode:overlay}"
        ".tr-light-leak .ll-blob{inset:-48% -38% 18% 12%;border-radius:50%;"
        "background:radial-gradient(circle at 78% 22%,"
        "rgba(255,255,255,0.95) 0%,rgba(229,50,45,0.78) 34%,"
        "rgba(255,106,95,0.28) 58%,transparent 74%);"
        "mix-blend-mode:screen}"
        ".tr-light-leak .ll-hot{inset:-18% -12% 62% 48%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(255,106,95,0.9) 0%,transparent 62%);"
        "mix-blend-mode:screen}"
        ...
        "rgba(255,106,95,0) 38%,rgba(255,106,95,0.88) 50%,"
        "rgba(229,50,45,0.5) 58%,transparent 78%);"
        "mix-blend-mode:screen;transform-origin:50% 50%}"
        ".tr-light-leak .ll-o0{inset:-36% -18% 48% 28%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(229,50,45,0.4) 0%,transparent 70%);"
        "mix-blend-mode:screen}"
        ".tr-light-leak .ll-o1{inset:-22% -8% 38% 8%;border-radius:50%;"
        "background:radial-gradient(circle,rgba(255,106,95,0.32) 0%,transparent 70%);"
        "mix-blend-mode:screen}"
        ...
        "transform-origin:50% 50%}"
        ".tr-sdf-iris .si-iris{background:#E5322D;mix-blend-mode:overlay}"
        ".tr-sdf-iris .si-ring{"
        "background:radial-gradient(circle,transparent 46%,"
        "rgba(255,106,95,0.92) 50%,transparent 54%);"
        "mix-blend-mode:screen}"
        ...
        ".tr-thermal-distortion .td-to{background:#E5322D;mix-blend-mode:overlay}"
        ...
        "rgba(255,106,95,0.92) 0%,rgba(229,50,45,0.55) 42%,transparent 70%);"
        "mix-blend-mode:screen}"
        ...
        "rgba(255,106,95,0.7) 50%,transparent 100%);"
        "mix-blend-mode:screen;transform-origin:50% 50%}"
        ...
        ".tr-whip-pan .wp-to{background:#E5322D;mix-blend-mode:overlay}"
        ...
        "rgba(255,106,95,0) 12%,rgba(229,50,45,0.88) 48%,"
        ...
        "background:linear-gradient(120deg,#F7F5F3 0%,#FF6A5F 50%,#E5322D 100%);"
        ...
        ".tr-transitions-3d .t3-b{background:#E5322D;opacity:0}"
        ...
        ".tr-transitions-blur .tb-b{background:#E5322D;opacity:0}"
        ...
        ".tr-transitions-cover .tc-b{background:#E5322D;opacity:0}"
        ...
        "opacity:0}"
        ".tr-transitions-cover .tc-wb{background:#9C1B17}"
        ".tr-transitions-cover .tc-wa{background:#E5322D}"
        ...
        ".tr-transitions-light .tlt-b{background:#E5322D;opacity:0}"
        ...
        "background:linear-gradient(135deg,rgba(229,50,45,0.6),transparent 60%);"
        "transform-origin:50% 50%}"
        ...
        ".tr-transitions-other .tto-b{background:#E5322D;opacity:0}"
...
# Inter, #0b0c0e/#f7f8fa и RGB-сдвиг #ff3158/#19e6d2 как в каталоге:
...
def fs_shared_axis_z(ctx: "TemplateCtx") -> Piece:
    ...
    Inter 900 и ``#18181b`` как в каталоге; ``tone=accent`` → ``#E5322D``,
...
def ov_lt_accent_underline(ctx: "TemplateCtx") -> Piece:
    ...
    вне списка. Мятный ``#46e5b7`` — чужой бренд, черта канала ``#E5322D``.
...
def ov_lt_clean_bar(ctx: "TemplateCtx") -> Piece:
    ...
    ``#E5322D``. Montserrat как в каталоге. Твины на маске, tab и строках,
    не на ``.clip``.
    """
    params = ctx.params
    name = str(params.get("name") or params.get("content") or params.get("text")
...
def ov_lt_dark_card(ctx: "TemplateCtx") -> Piece:
    ...
    Каталог твинит ``tl.to`` после ``gsap.set`` и прячет обёртку через
    ``visibility``. Движок требует ``fromTo`` на вложенных узлах; ``visibility``
    вне списка. Золото ``#f5b942`` — чужой бренд, черта канала ``#E5322D``.
...
def overlay_css(brandbook: dict[str, Any]) -> str:
    ...
    return (
        ...
        "border:2px solid rgba(229,50,45,0.35);"
        ...
        ".fullscreen-text.invert .fs-slam-punch{color:#E5322D;"
        "text-shadow:0 4px 22px rgba(229,50,45,0.35),0 3px 18px rgba(0,0,0,0.55)}"
        ...
        "text-shadow:0 0 24px rgba(229,50,45,0.45)}"
        ...
        ".fullscreen-text.fs-shared-axis-z.saz-accent{color:#E5322D}"
        ...
        "radial-gradient(ellipse 80% 60% at 50% 40%,rgba(229,50,45,0.10) 0%,"
        ...
        "box-shadow:0 40px 120px rgba(0,0,0,0.45),0 0 0 1px rgba(229,50,45,0.12),"
        ...
        "background:radial-gradient(circle,rgba(229,50,45,0.32) 0%,transparent 68%);"
        "pointer-events:none}"
        ...
        ".fullscreen-text .bfc-wave-path{fill:none;stroke:#E5322D;stroke-width:4;"
        "stroke-linecap:round;stroke-linejoin:round}"
        ".fullscreen-text .bfc-wave-fill{fill:#E5322D;opacity:0.12}"
        ...
        "background:linear-gradient(180deg,#E5322D 0%,rgba(229,50,45,0.25) 100%);"
        ...
        "background:#E5322D;padding:8px 16px;border-radius:999px}"
        ...
        "letter-spacing:0.22em;text-transform:uppercase;color:#E5322D}"
        ...
        ".fullscreen-text .bfc-b-value.accent{color:#E5322D}"
        ...
        "text-transform:uppercase;color:#ffffff;"
        "text-shadow:0 0 40px rgba(229,50,45,0.45),0 8px 40px rgba(0,0,0,0.55);"
        "will-change:transform,opacity}"
        ".fullscreen-text .bfc-outline{position:absolute;inset:48px;border:3px solid "
        "#E5322D;border-radius:8px;box-shadow:0 0 0 1px rgba(229,50,45,0.25),"
        "inset 0 0 0 1px rgba(229,50,45,0.15),0 0 48px rgba(229,50,45,0.2);"
        ...
        "linear-gradient(90deg,transparent 0%,rgba(229,50,45,0.10) 48%,"
        ...
        "color:#ffffff;background:#E5322D;padding:10px 18px;border-radius:6px;"
        ...
        "transparent 0%,rgba(229,50,45,0.18) 35%,rgba(17,18,20,0.35) 50%,"
        "rgba(229,50,45,0.18) 65%,transparent 100%);filter:blur(18px);"
        ...
        ".article-scroll.theme-dark .hl{background:rgba(229,50,45,.28);"
        "box-shadow:0 0 0 6px rgba(229,50,45,.28)}"
        ...
        ".lt-au-rule{display:block;height:6px;border-radius:3px;background:#E5322D;"
        ...
        ".lt-cb-tab{display:block;width:12px;flex-shrink:0;background:#E5322D;"
        ...
        "padding:24px 38px 26px 32px;border:1px solid rgba(229,50,45,0.4);"
        ...
        "white-space:nowrap;will-change:transform,opacity}"
        ".lt-dc-rule{display:block;height:4px;border-radius:2px;background:#E5322D;"
        "transform-origin:0% 50%;will-change:transform}"
