...
class SafeZones:
    ...
    def center_x(self) -> int:
        """Центр рабочей зоны — не центр кадра.

        Правая граница ужата под колонку лайк/коммент/шер (§3.2), поэтому
        середина рабочей зоны смещена влево от оптического центра кадра. Для
        элементов, которые обязаны стоять ровно по центру экрана (субтитры,
