...
def face_band_y(brandbook: dict[str, Any]) -> tuple[float, float]:
    band = (brandbook.get("avatar") or {}).get("face_band_y") or [350, 750]
...
def collision_rects(brandbook: dict[str, Any], *,
    ...
    """Bands the face must not occupy: caption strip + bottom 400."""
    return [
        ("caption", caption_layout_bbox(brandbook)),
...
def target_face_rect(brandbook: dict[str, Any], *,
    ...
    Mode A: brandbook ``face_band_y``, clipped so it never enters captions.
    ...
    cap = caption_layout_bbox(brandbook)
    bottom = int((brandbook.get("safe_zones") or {}).get("bottom_px", 400))
    right = int((brandbook.get("safe_zones") or {}).get("right_px", 250))
    cap_top = float(cap[1])
    y_limit = min(cap_top, float(height - bottom))
    if str(mode).upper() == "B":
        y0 = max(float(height) * 0.5, float(cap[3]))
        y1 = float(height - bottom)
        if y1 <= y0 + 8:
            y1 = y_limit
            y0 = max(0.0, y1 - (band_hi - band_lo))
        return (0.0, y0, float(max(1, width - right)), y1)
    y0 = band_lo
    y1 = min(band_hi, y_limit)
    if y1 <= y0:
        y1 = y_limit
        y0 = max(0.0, y1 - (band_hi - band_lo))
...
def fit_compose_zoom(
    ...
    *,
    ...
    tw = max(tx1 - tx0, 1.0)
    th = max(ty1 - ty0, 1.0)
    z = min(req, tw / fw, th / fh)
    out_w, out_h = fw * z, fh * z
    dest_x = tx0 + (tw - out_w) / 2.0
    dest_y = ty0 + max(0.0, (th - out_h) * 0.12)
    left = dest_x - sx1 * z
    top = dest_y - sy1 * z
