...
def pick_bed(cfg, *, want: Sequence[str], video_id: str,
    ...
    * **кольцо последних бедов** (``bed_ring``, §10.3) — прямой список id,
      прозвучавших в последних роликах. Счётчик ``used_in`` ротирует
    ...
    * **хэш ``video_id``** — на полном отпечатке, а не на первом байте.
    ...
    def rank(item: Any) -> tuple[int, int, int]:
        ...
        stale = 1 if (recent and set(item.used_in) & recent) or item.id in ring else 0
