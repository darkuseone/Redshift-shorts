...
* музыкальная подложка −24…−26 LUFS (доля ~0.28–0.32 от голоса): слышна
  как напряжение под речью, не конкурирует с VO;
...
def music_target_lufs(cfg) -> float:
    ...
    Заказчик задаёт его долей от голоса, и так честнее: подложка существует
    только относительно речи. Коридор продукта −24…−26 LUFS при voice −14
    это доля ≈ 0.28–0.32 (20·lg(0.282) ≈ −11 дБ от голоса).

    Доля амплитудная: 0.30 → 20·lg(0.30) = −10.5 дБ от голоса, при
    ``voice_lufs`` −14 это ≈ −24.5 LUFS.
    """
    ...
    lufs = cfg.get("audio.music_lufs", [-26, -24])
