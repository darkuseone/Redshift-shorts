...
description: Каталог из 142 шаблона, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

142 шаблон в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 142
...
(кольцо от 12 часов, бренд `#35d6a0`). Два числа, второе меньше первого —
...
`compare-bars` / `bar-race-mini`. Два–три числа без спада — `chart-story`. Не
...
142 шаблон — это **рендереры с параметрами**, а не 142 реализация:
