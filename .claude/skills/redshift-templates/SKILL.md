...
description: Каталог из 143 шаблона, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

143 шаблон в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 143
...
`mk-line-graph`. Не пересекается с CTA последних 2 сек.
...
143 шаблон — это **рендереры с параметрами**, а не 143 реализация:
