...
description: Каталог из 150 шаблона, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

150 шаблон в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 150
...
150 шаблон — это **рендереры с параметрами**, а не 147 реализация:
`fullscreen_text` и его варианты покрывают полноэкранные кадры, `source_card` /
`chat_thread` / `ai_chat_reveal` / `article_scroll` / `paper_reveal` — окна источника, `plaque` /
