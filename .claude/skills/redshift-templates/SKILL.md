...
description: Каталог из 182 шаблонов, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

182 шаблона в 12 категориях (§15). Каталог генерируется
...
182 шаблона — это **рендереры с параметрами**, а не 147 реализация:
...
`lt_accent_underline` / `lt_clean_bar` / `lt_dark_card` / `instagram_follow` — плашки, `kenburns` — проезд, `dataviz` — графики. Меняя шаблон, чаще всего
