...
description: Каталог из 144 шаблона, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

144 шаблон в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 144
...
`mk-line-graph`. Если в блоке Испания / Madrid / PIB — `spain-map`. Не пересекается с CTA последних 2 сек.
...
144 шаблон — это **рендереры с параметрами**, а не 144 реализация:
