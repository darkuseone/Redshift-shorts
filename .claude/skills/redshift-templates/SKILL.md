...
description: Каталог из 138 шаблонов, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

138 шаблонов в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 138
...
`stat-countup-card`, два и больше — `animated-bar-chart` (карточка
столбиков `scaleY`, KPI) с запасными `compare-bars` / `bar-race-mini`.
Не пересекается с CTA последних 2 сек.
...
`bar-race-mini` / `compare-bars` не трогаем.
...
138 шаблонов — это **рендереры с параметрами**, а не 138 реализаций:
