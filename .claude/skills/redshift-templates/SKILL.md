...
description: Каталог из 141 шаблона, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

141 шаблон в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 141
...
(кольцо от 12 часов, бренд `#35d6a0`). Четыре и больше — `bar-chart-race` (гонка рядов,
лидер `#c8452d`) с запасными `chart-story` / `animated-bar-chart` /
`compare-bars` / `bar-race-mini`. Два–три числа — `chart-story`. Не
пересекается с CTA последних 2 сек.
...
141 шаблон — это **рендереры с параметрами**, а не 141 реализация:
