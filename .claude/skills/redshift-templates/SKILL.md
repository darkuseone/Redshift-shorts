...
description: Каталог из 139 шаблонов, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

139 шаблонов в 12 категориях (§15). Каталог генерируется
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 139
...
лидер `#c8452d`) с запасными `animated-bar-chart` / `compare-bars` /
`bar-race-mini`. Два–три числа — `animated-bar-chart`. Не пересекается с
CTA последних 2 сек.
...
139 шаблонов — это **рендереры с параметрами**, а не 139 реализаций:
