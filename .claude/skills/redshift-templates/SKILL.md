...
description: Каталог из 178 шаблонов, ротация и различия версий A/B. Используй при работе с templates/, выборе шаблона, приёмах вокруг ведущего, ошибке AB_TOO_SIMILAR и проверке QC-17.
---
# redshift-templates

178 шаблонов в 12 категориях (§15). Каталог генерируется
...
| `browser-ui` | 14 | окна интерфейсов |
...
3. Редко используемые шаблоны получают приоритет: без этого каталог из 178
...
178 шаблонов — это **рендереры с параметрами**, а не 147 реализация:
`fullscreen_text` и его варианты покрывают полноэкранные кадры, `source_card` /
`chat_thread` / `ai_chat_reveal` / `chatgpt_exchange` / `claude_exchange` / `app_showcase` / `article_scroll` / `paper_reveal` — окна источника, `plaque` /
