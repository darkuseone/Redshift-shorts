...
**Футаж ищет и смотрит своим зрением режиссёр в чате** (по умолчанию Claude
Code) — всегда, в каждом ролике. Vision-API и ключ xAI для этого не
используются вообще.
...
| Magnific / Freepik stock | живая съёмка (животные, люди, города), логотипы, фото | лицензия подписки (`magnific`) | **MCP**: `stock_search` с `license: "free"` → `stock_download` — **0 кредитов** (проверено 22.09.2026: баланс не изменился), видео 720p. `creditCost` в выдаче — цена для платных позиций, на free не списывается. Ссылка живёт ~час → файл в репо. Сайт: до 100 бесплатных скачиваний/день (браузер GrokBot) |
...
3. xAI не используется (решение заказчика 22.09): ни генерация, ни vision.
