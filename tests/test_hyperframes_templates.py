...
def test_flowchart_vertical_keeps_catalog_tokens():
    ...
    # accent (#D7263D) — off-palette #9747ff and #0b84f3 removed.
    assert "#D7263D" in css  # accent replaces ex-purple tag and ex-blue selection border
...
def test_code_diff_css_keeps_github_diff_and_mono():
    ...
    assert "#D7263D" not in re.search(r"\.cd-del\{[^}]+\}", css).group(0)
    assert "#D7263D" not in re.search(r"\.cd-add\{[^}]+\}", css).group(0)
...
def test_beat_freeze_cut_keeps_it_kosmos_palette_not_catalog_mint():
    ...
    assert "stroke:#D7263D" in wave
    eyebrow = re.search(r"\.bfc-eyebrow\{[^}]+\}", css).group(0)
    assert "color:#D7263D" in eyebrow
    pill = re.search(r"\.bfc-pill\{[^}]+\}", css).group(0)
    assert "background:#D7263D" in pill
...
def test_transitions_radial_animates_without_webgl(ctx):
    """Каталог демонстрирует radial переходы; здесь круговое диафрагменное раскрытие."""
    piece = render_transition("transitions_radial", TemplateCtx(
        **{**ctx.__dict__, "params": {"from_scale": 1.10}}))
    node = piece.nodes[0]
    assert "tr-transitions-radial" in node
    assert "trad-stage" in node
    assert "trad-a" in node and "trad-b" in node
    assert "trad-iris" in node
    assert "trad-ring" in node
    assert "trad-blur" in node
    assert node.count(f'id="tr-{ctx.index:02d}"') == 1
    body = " ".join(piece.tweens)
    assert "scale:1.1" in body or "scale:1.10" in body
    assert f'"#{ctx.target}"' in body
...
def test_transitions_radial_keeps_catalog_tokens():
    from src.lib.config import load_config
...
def test_mk_clone_wall_keeps_catalog_ink_paper_and_blobs():
    ...
    assert "#D7263D" in css.split(".tr-mk-clone-wall", 1)[1].split(".fullscreen-text", 1)[0]
...
def test_transitions_3d_keeps_catalog_navy_and_terracotta():
    ...
    assert "#D7263D" in css.split(".tr-transitions-3d", 1)[1]
...
def test_transitions_blur_keeps_catalog_navy_and_terracotta():
    ...
    assert "#D7263D" in css.split(".tr-transitions-blur", 1)[1]
...
def test_transitions_cover_keeps_catalog_magenta_and_purple():
    ...
    assert "#111214" in face_a
    assert "#D7263D" in face_b
    assert "#D7263D" in wipe_a
    assert "#8E1627" in wipe_b
...
def test_lt_accent_underline_css_keeps_oswald_and_remaps_mint():
    ...
    assert ".lt-au-rule{display:block;height:6px;border-radius:3px;background:#D7263D;" in css
...
def test_lt_clean_bar_css_keeps_montserrat_and_remaps_orange():
    ...
    assert "background:#D7263D" in tab
...
def test_lt_dark_card_css_keeps_charcoal_montserrat_and_remaps_gold():
    ...
    assert "background:#D7263D" in rule
    assert "transform-origin:0% 50%" in rule
    assert "transform:" not in rule.replace("transform-origin:0% 50%", "")
    assert "Montserrat" in css
    assert "#ffffff" in css and "#7A7D82" in css
