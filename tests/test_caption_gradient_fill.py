...
def test_missing_caption_key_falls_back_to_the_brandbook(cfg):
    """Без явного жеста в плане подпись берётся из брендбука.

    Умолчание канала — «glow»: белое слово Montserrat Black с красным гало со
    скриншота заказчика. Жест из ветки шаблонов остаётся альтернативой, и
    выбирают его планом, а не тем, что он был умолчанием в своей ветке.
    """
    plan = _plan(_words("пиши", "html"))
    del plan["subtitle_style"]["caption"]
    out = CompositionBuilder(plan, cfg.brandbook, {}).build("assets/mix.wav")
    assert cfg.brandbook["subtitles"]["caption"] == "glow"
    assert 'class="clip word' in out
    assert "caption-grad" not in out
...
def test_css_has_no_clip_path_or_filter(cfg):
    ...
    # `.word` — стили жеста glow; они живут в той же таблице, потому что
    # умолчание канала именно glow, а gradient-fill стоит рядом.
...
def test_space_category_picks_clip_wipe_when_the_brandbook_asks_for_a_gesture(cfg):
    """Тема выбирает жест только там, где брендбук сам просит альтернативу.

    Канал про космос целиком, и правило «в теме есть космос — другой шрифт»
    без этой оговорки переписало бы подпись во всех роликах разом.
    """
    plan = {"category": "space", "title": "Чип"}
    assert is_space_theme(plan)
    assert pick_caption_style(plan, cfg.brandbook) == "glow"
    book = {**cfg.brandbook,
            "subtitles": {**cfg.brandbook["subtitles"], "caption": "gradient-fill"}}
    assert pick_caption_style(plan, book) == "clip-wipe"


def _gesture_book(cfg):
    """Брендбук, в котором умолчание — жест курсора, а не гало канала."""
    return {**cfg.brandbook,
            "subtitles": {**cfg.brandbook["subtitles"], "caption": "gradient-fill"}}
...
def test_cosmic_topic_picks_clip_wipe_without_space_category(cfg):
    ...
    assert is_space_theme(plan)
    assert pick_caption_style(plan, _gesture_book(cfg)) == "clip-wipe"
...
def test_ai_topic_stays_on_gradient_fill(cfg):
    ...
    assert pick_caption_style(plan, _gesture_book(cfg)) == "gradient-fill"
